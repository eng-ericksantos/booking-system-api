const swaggerSpec = {
  ...
  paths: {
    // ========= HealthCheck =========
    '/health': {
      get: { tags: ['HealthCheck'], summary: 'Verifica a conexão da API', responses: { '200': { description: 'Verifica a conexão da API Servidor e Banco de Dados', content: { 'application/json': { schema: { type: 'string', items: { $ref: '#/components/schemas/HealthCheck' } } } } } } },
    },
    // ========= Autenticação =========
    '/login': {
      post: {
        ...
        responses: {
          '200': {
            description: 'Login bem-sucedido', content: {
              'application/json': {
                schema: {
                  type: 'object',
                  properties: {
